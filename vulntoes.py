        ...
        def toES(self):
                ...
                for rh in report:
                        ...
                        for tag in rh.getchildren():
                                ...
                                self.es.index(index=self.index_name,doc_type="vuln", body=json.dumps(dict(list(host_item.items())+list(dict_item.items()))))
...
class NmapES:
        ...
        def toES(self):
                ...
                for h in self.root.iter('host'):
                        ...
                        print(self.input_file.split('.')[0])
                        tag = self.input_file.split('.')[0].split('/')[1]
                        dict_item['tag'] = tag
                        if h.tag == 'host':
                                if h.attrib['endtime']:
                                                ...
                                                if port.tag == 'port':
                                                        ...
                                                        for p in list(port):
                                                                if p.tag == 'state':
                                                                        ...
                                                                        if p.attrib['id'] == 'vulners':
                                                                            #print(p.attrib['id'])
                                                                            for line in str(p.attrib['output']).split('\n'):
                                                                                if 'CVE' in line:
                                                                                    cve = line.split()[0]
                                                                                    year = cve[4:8]
                                                                                    section = cve[9:-3] + 'xxx'
                                                                                    #url = """https://raw.githubusercontent.com/CVEProject/cvelist/master/{}/{}/{}.json""".format(year, section, cve)
                                                                                   # print('cvelist/'+year+'/'+section+'/'+cve+'.json')
                                                                                    with open('cvelist/'+year+'/'+section+'/'+cve+'.json') as json_file:
                                                                                        cve_json = json.load(json_file)
                                                                                    severity = NmapES.convert_severity(float(line.split()[1]))
                                                                                    refer = cve_json["description"]["description_data"][0]["value"]
                                                                                    #print(line.split())
                                                                                    cves['vulners'].append({"CVE" : cve,"severity": severity,"refer": refer})
                                                                            dict_item.update(cves)
...
class OpenVasES:
        ...
        def __createIssuesList(self):
                ...
                for result in self.root.iter('result'):
                        ...
                        if result.find('host') is not None:
                                issueDict['ip'] = str(result.find('host').text)
                                #print issueDict['host']
                        for nvt in result.iter('nvt'):
                                issueDict['oid'] = str(nvt.attrib['oid'])
                                for child in nvt:
                                        issueDict[child.tag] = str(child.text)

                        if result.find('description') is not None:
                                issueDict['description'] = str(result.find('description').text)
                        if result.find('port') is not None:
                                issueDict['port'] = str(result.find('port').text)
                        if result.find('threat') is not None:
                                issueDict['threat'] = str(result.find('threat').text)
                        if result.find('severity') is not None:
                                issueDict['severity'] = str(result.find('severity').text)
                        if result.find('scan_nvt_version') is not None:
                            issueDict['scan_nvt_version'] = str(result.find('scan_nvt_version').text)
                        if issueDict:
        ...
        def parsePort(self,string):
                ...
                #print string
                if portsParsed:
                        fieldsDict['service'] = str(portsParsed.group(1))
                        fieldsDict['port'] = str(portsParsed.group(2))
                        fieldsDict['protocol'] = str(portsParsed.group(3))
                        #print fieldsDict
                        return fieldsDict
                return None
